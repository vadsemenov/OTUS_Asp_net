...
2. Оформление заказа:
   ...
   - OrderService отправляет асинхронный запрос в StoreService (Сервис хранения товаров), чтобы забронировать товара на складе через брокер сообщений.
   - StoreService асинхронно через брокер сообщений отправляет информацию о успешном бронировании товара, если товар есть в наличии.
...
3. Планирование доставки:
   ...
   - DeliveryService получает запрос от OrderService через брокер сообщений и обрабатывает его. Сервис решает, какой транспорт и водителя выделить для доставки.
